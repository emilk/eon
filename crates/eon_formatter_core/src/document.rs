...
impl<'stream, 'source> Analyzer<'stream, 'source> {
    ...
    fn parse_value(&self, start: usize) -> Result<ValueSpan> {
        ...
        match token.kind {
            TokenKind::OpenList | TokenKind::OpenBrace => {
                self.scan_balanced_value(start).map(|end| ValueSpan::new(start, end))
            }
    ...
    fn scan_balanced_value(&self, start_open: usize) -> Result<usize> {
        ...
        expected_closers.push(matching_close(self.stream.token(start_open).unwrap().kind).ok_or(
            self.error_at_token(
                start_open,
...
mod tests {
    ...
    fn detects_root_implicit_map_entries() {
        ...
        assert_eq!(document.kind(), DocumentKind::ImplicitMap);
        assert_eq!(entry_token_texts(&stream, document.entries()), vec![
            ("alpha", "1"),
            ("beta", "[2]"),
        ]);
    }
    ...
    fn detects_root_implicit_list_items() {
        ...
        assert_eq!(value_token_texts(&stream, document.values()), vec!["1", "2", "3"]);
    }
    ...
    fn value_token_texts<'a>(
        ...
        values.iter().map(|value| span_text(stream, *value)).collect()
    }
