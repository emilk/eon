...
fn main() {
    let matches = Command::new("Con formatter")
        .about("Format Con files")
        .arg(
            Arg::new("files")
...
fn process_file(path: &Path, check_mode: bool) -> Result<bool, Box<dyn std::error::Error>> {
    ...
    let options = con_syntax::FormatOptions::default();
    let formatted = con_syntax::reformat(&content, &options)?;
